 ...
 * behind LEADS_CODE). Same renderer as the web version (src/lib/leads.mjs).
 *
 * Usage:
...
import { renderBrandsPage, STATUSES } from "../lib/leads.mjs";
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === "/status" && req.method === "POST") {
      ...
      res.writeHead(303, { Location: `/#${slug}` });
      res.end();
      return;
    }
    if (url.pathname === "/") {
      res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
      res.end(await renderBrandsPage({ postPath: "/status", logoutPath: "/" }));
      return;
    }
